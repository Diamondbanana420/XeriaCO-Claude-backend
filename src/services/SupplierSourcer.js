...
class SupplierSourcer {
  ...
  // ═══════════════════════════════════════
  // Brave Search — Supplier Finding Fallback
  // ═══════════════════════════════════════
  async findSupplierViaBrave(product) {
    if (!config.braveSearch?.apiKey) return null;

    try {
      const query = encodeURIComponent(`${product.title} dropshipping supplier wholesale`);
      const res = await axios.get(`https://api.search.brave.com/res/v1/web/search?q=${query}&count=10`, {
        headers: { 'X-Subscription-Token': config.braveSearch.apiKey },
      });

      const results = res.data?.web?.results || [];
      const supplierDomains = ['aliexpress.com', 'cjdropshipping.com', 'alibaba.com'];

      for (const result of results) {
        const url = result.url || '';
        const matchedDomain = supplierDomains.find(d => url.includes(d));
        if (matchedDomain) {
          const platform = matchedDomain.replace('.com', '').replace('.', '');
          return {
      ...
      logger.warn(`SupplierSourcer: Brave Search failed for "${product.title}" — ${err.message}`);
    }
  ...
  async findBestSupplier(product) {
    ...
    if (allResults.length === 0) {
      logger.info(`SupplierSourcer: No CJ/Ali suppliers found for "${query}", trying Brave Search`);
      const braveResult = await this.findSupplierViaBrave(product);
      if (braveResult) {
        logger.info(`SupplierSourcer: Found supplier via Brave Search — ${braveResult.platform}`);
        return { ...braveResult, score: 10 };
      }
      return null;
    }
